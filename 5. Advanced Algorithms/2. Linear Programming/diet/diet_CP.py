# python3
from sys import stdin
import copy
import numpy as np

infinite = (10**9)


class Equation:
    def __init__( self, a, b ):
        self.a = a
        self.b = b

class Position:
    def __init__( self, column, row ):
        self.column = column
        self.row = row


def FindPivot( a, used_rows, used_columns ):
    m = len(a)
    pivot_element = Position(0, 0)
    while used_rows[pivot_element.row]:
        pivot_element.row += 1
    while used_columns[pivot_element.column]:
        pivot_element.column += 1
    while 0 == a[pivot_element.row][pivot_element.column] or used_rows[pivot_element.row]:
        pivot_element.row += 1
        if pivot_element.row > m - 1:
            return False, None
    return True, pivot_element


def SwapLines( a, b, used_rows, pivot_element ):
    a[pivot_element.column], a[pivot_element.row] = a[pivot_element.row], a[pivot_element.column]
    b[pivot_element.column], b[pivot_element.row] = b[pivot_element.row], b[pivot_element.column]
    used_rows[pivot_element.column], used_rows[pivot_element.row] = used_rows[pivot_element.row], used_rows[
        pivot_element.column]
    pivot_element.row = pivot_element.column


def ProcessPivotElement( a, b, pivot_element ):
    n = len(a)
    m = len(a[pivot_element.row])
    d = a[pivot_element.row][pivot_element.column]

    for j in range(m):
        a[pivot_element.row][j] /= d
    b[pivot_element.row] /= d

    for i in range(n):
        if i != pivot_element.row:
            d = a[i][pivot_element.column]
            for j in range(pivot_element.column, n):
                a[i][j] -= a[pivot_element.row][j] * d
            b[i] -= b[pivot_element.row] * d


def MarkPivotElementUsed( pivot_element, used_rows, used_columns ):
    used_rows[pivot_element.row] = True
    used_columns[pivot_element.column] = True


def SolveEq(equation):
    a = equation.a
    b = equation.b
    size = len(a)

    used_columns = [False] * size
    used_rows = [False] * size
    for step in range(size):
        solved, pivot_element = FindPivot(a, used_rows, used_columns)
        if not solved:
            return False, None
        SwapLines(a, b, used_rows, pivot_element)
        ProcessPivotElement(a, b, pivot_element)
        MarkPivotElementUsed(pivot_element, used_rows, used_columns)

    return True, b


def addEq( n, m, A, b, infinite ):
    for i in range(m):
        equation = [0.0 for x in range(m)]
        equation[i] = -1.0
        A.append(equation)
        b.append(0.0)
    A.append([1.0 for x in range(m)])
    b.append(infinite)


def checkResult( n, mm, A, b, c, result, lastEquation, ans, bestScore ):
    for r in result:
        if r < -1e-3:
            return False, ans, bestScore
    for i in range(n):
        r = 0.0
        for j in range(mm):
            r += A[i][j] * result[j]
        if r > b[i] + 1e-3:
            return False, ans, bestScore
    score = 0.0
    for j in range(mm):
        score += c[j] * result[j]
    if score <= bestScore:
        return False, ans, bestScore
    else:
        if lastEquation:
            return True, 1, score
        else:
            return True, 0, score


def solve_diet_problem( n, mm, A, b, c, inf = infinite ):
    addEq(n, mm, A, b, infinite)
    l = n + mm + 1
    ans = -1
    bestScore = -float('inf')
    bestResult = None
    for x in range(2 ** l):
        used = [i for i in range(l) if ((x / 2 ** i) % 2) // 1 == 1]
        if len(used) != mm:
            continue
        lastEquation = False
        if used[-1] == l - 1:
            lastEquation = True
        As = [A[i] for i in used]
        bs = [b[i] for i in used]
        solved, result = SolveEq(copy.deepcopy(Equation(As, bs)))
        if solved:
            isAccepted, ans, bestScore = checkResult(n, mm, A, b, c, result, lastEquation, ans, bestScore)
            if isAccepted:
                bestResult = result

    return [ans, bestResult]


def solve_diet_problem0( n, mm, A, b, c ):
    res = linprog(-np.array(c), A, b)
    if res.status == 3:
        anst = 1
        ansx = None
    elif res.status == 0:
        anst = 0
        ansx = list(res.x)
    else:
        ans = -1
        andx = None
    return anst, ansx

def read_data():
    n, m = list(map(int, stdin.readline().split()))
    A = []
    for i in range(n):
      A += [list(map(int, stdin.readline().split()))]
    b = list(map(int, stdin.readline().split()))
    c = list(map(int, stdin.readline().split()))
    return n,m,A,b,c

def read_data_test():
    lines = open("tests/36",'r').readlines()
    n,m =list(map(int, lines[0].split()))
    A=[]
    for i in range(1,n+1):
        A+=[list(map(int,lines[i].split()))]
    b = list(map(int,lines[n+1].split()))
    c = list(map(int,lines[n+2].split()))
    return n,m,A,b,c

if __name__ == "__main__":
    #n,m,A,b,c = read_data_test()
    n,m,A,b,c = read_data()

    anst, ansx = solve_diet_problem(n, m, A, b, c)

    if anst == -1:
        print("No solution")
    if anst == 0:
        print("Bounded solution")
        print(' '.join(list(map(lambda x: '%.18f' % x, ansx))))
    if anst == 1:
        print("Infinity")